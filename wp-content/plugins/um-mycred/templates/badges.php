<span class="um-badges">
    <?php foreach ( $all_badges as $badge_id ) {
        ...
        <?php
        $badge_level_reached = mycred_badge_level_reached( um_profile_id() , $badge_id );
        ...
        if( ! empty( $image_url ) ) { ?>
            <span class="the-badge">
                <img src="<?php echo esc_attr( $image_url ) ?>"
                     ...
                     class="mycred-badge earned um-tip-n <?php echo $class ?>" />
            </span>
        <?php }
            ...
            if ( $level['attachment_id'] > 0 ){
                    $image_identification 	= $level['attachment_id'];
                            ...
                            if( ! empty( $level_title  ) ){
                                ...
                                $badge_title 	= "{$badge->title} - Level {$level_key__count}";
                            }
            }else{
                    $image_identification 	= $level['attachment_id'];
            }
            ...
            if( ! empty( $base_requirements ) ){
                    foreach( $base_requirements as $requirement ){
                        if( ( $badge_level_reached !== false && $level_key <= $badge_level_reached ) ){ // Unlocked
                            $badge_title .= sprintf( _x( '&#013;&#10;%s %s for "%s"', '"Gained/Lost" "x points" for "reference"', 'mycred' ), ( ( $requirement['amount'] < 0 ) ? __( 'Lost', 'mycred' ) : __( 'Gained', 'mycred' ) ), $mycred->format_creds( $requirement['amount'] ), $requirement['ref'] );
                        }else{ // Locked
                            $badge_title .= sprintf( _x( '&#013;&#10;%s %s for "%s"', '"Gain/Lost" "x points" for "reference"', 'mycred' ), ( ( $requirement['amount'] < 0 ) ? __( 'Lost', 'mycred' ) : __( 'Gain', 'mycred' ) ), $mycred->format_creds( $requirement['amount'] ), $requirement['ref'] );
                        }
                    }
            }
